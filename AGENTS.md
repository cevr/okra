...
- Image's `--ref <path>` (repeatable, `Flag.atLeast(0)`) is a **style/composition reference**, not an edit. It's **codex-only**: `/images/generations` has no input-image field, so `--ref` + an OpenAI image model fails `INVALID_INPUT`. The command reads each ref (`refMediaType` sniffs png/jpg/jpeg/webp/gif from the extension) and `ImageGen.generate` builds a structured prompt — a user message with the text plus each ref as a `file` part (image media type), which the OpenAI Responses adapter forwards as `input_image`. No refs → plain-string prompt (unchanged path). Pixel-level editing (`/images/edits`) is a deferred future `--edit` mode

...
- Image's `--edit` (boolean) / `--mask <path>` / `--fidelity high|low` are **GPT-image-only** edit controls. `resolveRoute` (pure, in `commands/index.ts`) reconciles model+flags into `"codex" | "openai-generate" | "openai-edit"` or an `INVALID_INPUT` (returned as a `{ok,...}` result, NOT thrown — avoids `instanceof` on a Schema class). Rules: codex + `--edit`/`--mask`/`--fidelity` → error (no pixel-edit primitive; "use gpt-image-1.5"); OpenAI + any input image (`--ref`/`--edit`/`--mask`) → edit; `--edit`/`--mask` without `--ref` → error (needs source); `dall-e-*` + ref → error (`supportsEdits` = `gpt-image` prefix only). `--fidelity` is edits-only + `supportsInputFidelity` (gpt-image-1 / gpt-image-1.5, NOT `-mini`) — errors otherwise; maps to the `input_fidelity` multipart field (API default `low`). NO auto-switch: `--edit` on the codex default errors rather than silently switching to the metered API

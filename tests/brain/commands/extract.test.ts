...
describe("extract", () => {
  describe("parsing", () => {
    it.scoped("parses string content messages", () =>
      Effect.gen(function* () {
    ...
    it.scoped("parses content arrays [{type: 'text', text: '...'}]", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const inputDir = `${dir}/input`;
    ...
    it.scoped("parses codex response_item messages", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const inputDir = `${dir}/input`;
  ...
  describe("filtering", () => {
    it.scoped("skips system-reminder-only messages", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const inputDir = `${dir}/input`;
    ...
    it.scoped("skips isMeta: true messages", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const inputDir = `${dir}/input`;
    ...
    it.scoped("skips small files (<500 bytes)", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const inputDir = `${dir}/input`;
    ...
    it.scoped("preserves messages with subType: thinking", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const inputDir = `${dir}/input`;
    ...
    it.scoped("skips tool_use and tool_result subTypes", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const inputDir = `${dir}/input`;
  ...
  describe("date filtering", () => {
    it.scoped("--from filters out conversations before the date", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
    ...
    it.scoped("--to filters out conversations after the date", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const path = yield* Path;
    ...
    it.scoped("--from and --to together select a date range", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const path = yield* Path;
  ...
  describe("output", () => {
    it.scoped("sorts conversations newest-first", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const path = yield* Path;
    ...
    it.scoped("formats output as [USER]: / [ASSISTANT]:", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const inputDir = `${dir}/input`;
    ...
    it.scoped("creates batch manifests", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const path = yield* Path;
    ...
    it.scoped("0 conversations produces empty batchPaths", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem;
        const dir = yield* fs.makeTempDirectoryScoped();
        const inputDir = `${dir}/input`;

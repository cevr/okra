...
describe("VaultService", () => {
  describe("init", () => {
    it.live("creates directories and seed files", () =>
      withTempDir((dir) =>
        Effect.gen(function* () {
    ...
    it.live("is idempotent — second call creates nothing", () =>
      withTempDir((dir) =>
        Effect.gen(function* () {
          const vault = yield* VaultService;
  ...
  describe("listFiles", () => {
    it.live("returns .md files sans extension, excludes index.md and node_modules", () =>
      withTempDir((dir) =>
        Effect.gen(function* () {
          const vault = yield* VaultService;
          const fs = yield* FileSystem;
  ...
  describe("rebuildIndex", () => {
    it.live("generates correct index with sections", () =>
      withTempDir((dir) =>
        Effect.gen(function* () {
          const vault = yield* VaultService;
          const fs = yield* FileSystem;
    ...
    it.live("is no-op when unchanged", () =>
      withTempDir((dir) =>
        Effect.gen(function* () {
          const vault = yield* VaultService;
          const fs = yield* FileSystem;
    ...
    it.live("strips wikilink anchors in comparison", () =>
      withTempDir((dir) =>
        Effect.gen(function* () {
          const vault = yield* VaultService;
          const fs = yield* FileSystem;
    ...
    it.live("includes standalone files under Other section", () =>
      withTempDir((dir) =>
        Effect.gen(function* () {
          const vault = yield* VaultService;
          const fs = yield* FileSystem;
  ...
  describe("status", () => {
    it.live("returns correct file count, sections, orphans", () =>
      withTempDir((dir) =>
        Effect.gen(function* () {
          const vault = yield* VaultService;
          const fs = yield* FileSystem;
  ...
  describe("snapshot", () => {
    it.live("concatenates files with delimiters", () =>
      withTempDir((dir) =>
        Effect.gen(function* () {
          const vault = yield* VaultService;
          const fs = yield* FileSystem;
    ...
    it.live("creates parent directories for output", () =>
      withTempDir((dir) =>
        Effect.gen(function* () {
          const vault = yield* VaultService;
          const fs = yield* FileSystem;
